...
def save_emotion_scores(names, vals, aros, output_directory, subject_id, session):
    ...
    df.to_csv(op.join(output_directory, f"emoscoring_SOUNDS_Pp{subject_id}_s_{session}.csv"), index=False)
...
def main():
    ...
    audio_paths = filtered_df['Audio'].tolist()
    print(audio_paths)
    ...
    instr0_path = op.join(image_dir, 'instr1_audio.png')
    end_path = op.join(image_dir, 'end.png')
    instr2_path = op.join(image_dir, 'instr2_audio.png')
    instr3_path = op.join(image_dir, 'instr3_audio.png')
