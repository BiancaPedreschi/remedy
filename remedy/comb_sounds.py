...
def generate_combinations(participant_id, categories, audio_labels, base_audio_path):
    ...
    numeric_info= sorted([2753.0, 3303.0, 4953.0, 6602.0, 7152.0, 8802.0], reverse=True)
    # Crea i percorsi dei file audio basati sulle etichette
    audio_paths = [os.path.join(base_audio_path, f"_audio_{num}_{label}.wav") for num, label in zip(numeric_info, participant_audio_labels)]
...
def main():
    ...
    base_audio_path = '/Users/foscagiannotti/Desktop/python_projects/space/remedy/data/remedy_data/sounds_test_2secv1'  # Modifica con il percorso corretto
   ...
   # Verifica il bilanciamento delle combinazioni
    ...
    audio_counts = all_combinations.groupby(['Session', 'Audio']).size().unstack(fill_value=0)
    ...
    print("\nCounts per Audio:")
    print(audio_counts)
    all_combinations.to_csv('all_combinations_audio_2sec_v1.csv', index=False)
