...
def save_emotion_scores(names, vals, aros, disting, famil, rips, output_directory, subject_id, session):
    ...
    df.to_csv(op.join(output_directory, f"PSEUDOWORDS_EMO_SCORING_Pp{subject_id}_s_{session}.csv"), index=False)
...
def main():
    ...
    base_audio_path = op.join(data_dir, 'pseudowords_test')
    audio_paths = [op.join(base_audio_path, f'pseudoparola_{i}.wav') for i in range(1, 10)]
