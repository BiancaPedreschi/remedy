...
#                 rem_stimulus, stream_pink, audio_pink, dev_hp, dev_sp, stim_n):
    ...
    if stimulation_started:
        ...
        instruction_text.draw()
        win.flip()    
        dreamquestrc(subject_id, session, sex, dev_sp, fs=44100)
...
def task_E():
    ...
    # pink_noise_file = os.path.join(data_dir, 'pwd', 'night_stim', 'PN.wav')
    pink_noise_file = os.path.join(data_dir, 'PN_48000.wav')
    ...
    try:
        while stimulation_count < max_stimulations:
            ...
            result = manual_stim(p, win=win, kb=kb, subject_id=subject_id,
