...
def main(data_dir, olfm, olf_commands, odor_names):
    ...
    win = visual.Window(fullscr=False, size=(widthPix, heightPix), 
                        color="grey", units='pix', monitor=mon, pos=(0, -0.2), 
                        screen=scrn, winType="pyglet")
    ...
    # kb = None
    emoKeys = ['1', 'num_1', '2', 'num_2', '3', 'num_3', '4', 'num_4', '5', 
               'num_5', 'escape']
    ...
    for nOdor, command in enumerate(olf_commands):
        names.append(odor_names[nOdor])
        ...
        slide_time1 = visual.TextStim(win, text="Premi la FRECCIA DESTRA" +
                                                " quando SENTI l'odore",
                                     units="norm", pos=(0, 0), color="black")
        slide_time2 = visual.TextStim(win, text="Premi la FRECCIA DESTRA " +
                                                "quando NON senti più l'odore",
                                      units="norm", pos=(0, 0), color="black")
        show(slide_odor)
        core.wait(2)
        ...
        while experiment_clock.getTime() < 30.:
            if not  first_press_detected:
                ...
                if keyName == "right":
                    ...
                    print(f"First key pressed: right, Time: " +
                          f"{first_press_time}")
                elif keyName == "escape":
                    core.quit()
            else:
        ...
        while experiment_clock.getTime() < 120:
            if not second_press_detected:
                ...
                if keyName == "right":
                    ...
                    print(f"Second key pressed: right, Time: " +
                          f"{second_press_time}")
                elif keyName == "escape":
                    core.quit()
            else:
    ...
    save_emotion_scores(names, vals, aros, commands, space_press_times, 
                        second_space_press_times)
...
if __name__ == "__main__":
    ...
    manualcomm_path = op.join(parent_dir, 'combinations', 
                              'manual_commands.csv')
    ...
    current_day = time.strftime('%d%m%Y')
    output_filename = (f'stream_stimolf_Pp{subject_id}_test{odor1, odor2}' +
                       f'_{current_day}.txt')
    ...
    # --------------------------  EXPERIMENT START  --------------------------
    main(data_dir, olfm, commandList, namesList)
    # --------------------------  EXPERIMENT END  --------------------------
